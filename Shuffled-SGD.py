...
# epsilon_list = 100*np.ones([num_batches,1]) #non-private
# epsilon_list = np.random.uniform(low=0.01, high=1, size=(num_batches, 1)) #Unif 1
# epsilon_list = np.random.uniform(low=0.01, high=2, size=(num_batches, 1)) #Unif 2
epsilon_list = np.random.uniform(low=2, high=2, size=(num_batches, 1))  # Unif 3
# epsilon_list = 0.5*np.ones([num_batches,1]) #constant
# epsilon_list = np.concatenate((np.ones(num_batches//2), 0.01 * np.ones(num_batches//2)))
# np.random.shuffle(epsilon_list) #mixed

# epsilon_list = np.clip(np.random.normal(0.5, 0.2, size=(num_batches, 1)),0.05,1) #Gauss 1
# epsilon_list = np.clip(np.random.normal(1, 0.2, size=(num_batches, 1)),0.01,2) #Gauss 2
# epsilon_list = np.clip(np.random.normal(1, 0.5, size=(num_batches, 1)),0.01,2) #Gauss 3
